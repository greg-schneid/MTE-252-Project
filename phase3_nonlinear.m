...
function split_signals = split_frequency(n, sample_rate, signal)
    ...
    % Logarithmic spacing: frequencies are spaced logarithmically
    % This mimics the tonotopic organization of the cochlea
    log_low = log10(low_freq);
    log_high = log10(hi_freq);
    log_freqs = linspace(log_low, log_high, n + 1);
    freq_edges = 10 .^ log_freqs;
    
    for i = 1:n
        band_low = freq_edges(i);
        band_high = freq_edges(i + 1);
...
function cosine_signals = generate_channel_cosines(n, sample_rate, envelope_signals)
    ...
    for i = 1:n
        % Calculate the central frequency of this channel's bandpass filter (geometric mean)
        band_low = freq_edges(i);
        band_high = freq_edges(i + 1);
        center_freq = sqrt(band_low * band_high); % Geometric mean for log spacing
